...
def test_category_class_attributes(category_fixture):
...
def test_category(category_fixture):
    ...
    assert category_fixture[1].description == "Современный телевизор, который позволяет наслаждаться просмотром, станет вашим другом и помощником"
    assert len(category_fixture) == 2
    assert len(category_fixture[0].products) == 3
    assert len(category_fixture[1].products) == 1

def test_samsung_product_price(category_fixture):
...
def test_total_products_in_all_categories(category_fixture):
